...
public class BookmarkCourseListItemAdapter extends RecyclerView.Adapter<BookmarkCourseListItemAdapter.CustomViewHolder> {
    ...
    @NonNull
    @Override
    public BookmarkCourseListItemAdapter.CustomViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        // 뷰 홀더를 위한 뷰 생성
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.bookmark_course_list, parent,false);
        // 뷰 홀더 객체 생성
        BookmarkCourseListItemAdapter.CustomViewHolder holder = new BookmarkCourseListItemAdapter.CustomViewHolder(view);
        return holder;
    }

    // 뷰 홀더의 내용을 변경하는 함수
    @Override
    public void onBindViewHolder(@NonNull BookmarkCourseListItemAdapter.CustomViewHolder holder, int position) {
        // 뷰 홀더에 들어갈 데이터를 가져옴
    ...
    public class CustomViewHolder extends RecyclerView.ViewHolder {
        ...
        public CustomViewHolder(@NonNull View itemView) {
            super(itemView);
            this.cosImageView = (ImageView)itemView.findViewById(R.id.cosImageView);
            this.cosName = (TextView) itemView.findViewById(R.id.cosName);
            this.cosLocation = (TextView) itemView.findViewById(R.id.cosLocation);
            this.cosMenuBtn = (ImageView) itemView.findViewById(R.id.cosMenuBtn);
        }
    }
}
